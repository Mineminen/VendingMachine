...
<input name="search" type="submit" border="0" class="btn btn-success btn-w-md "	value='搜索' />
    ...
    <table style="width:100%;text-align: center" id="show_tab">
        ...
        <tr>
        <td>${news.title}</td>
        <td>${fn:substring(news.nmemo, 0, 30)}</td>
        <td>${news.naddtime}</td>
<td>
        <a href="<%=path %>/tonewsView.action?id=${news.id}" class="cu-tag bg-cyan light "><i class="glyphicon glyphicon-zoom-in"></i>详细</a>&nbsp;
        <a href="<%=path %>/tonewsEdit.action?id=${news.id}" class="cu-tag bg-green light "><i class="glyphicon glyphicon-pencil"></i>编辑</a>&nbsp;
        <a href="javascript:void(0)" onclick="layer.confirm('您确定要删除吗？',{icon:3,title:'提示'},function(index){
            layer.close(index);
            window.location.href='<%=path %>/newsDel.action?id=${news.id}';
